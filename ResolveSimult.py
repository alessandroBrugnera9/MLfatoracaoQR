...
def Resolver_simult (m, n, p, W, A):
    ...
    def Rotgivens(W,n,m,i,j,c,s):     #C�digo do RotGivens fornecido pelo IME
        W[i,0:m] , W[j,0:m] = c * W[i,0:m] - s * W[j,0:m] , s * W[i,0:m] + c * W[j,0:m]
        return W
	
    
    def Triangularizar (W, A):       #Aplicar RotGivens at� que W fique traiangular
        W_tmp = copy.deepcopy(W)     #E aplica RotGivens em A tamb�m
        A_tmp = copy.deepcopy(A)
        for k in range(p):
    ...
    def ResolvSimult (m, R, A):  #Dado a matriz triangular R e o A aplicado... 
        ...
        for k in range(p-1, -1, -1):
               if R[k,k] != 0:   #impedir divis�o por 0
                   for j in range(m):
                       somatorio = 0
